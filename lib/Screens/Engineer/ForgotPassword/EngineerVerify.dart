...
import 'package:ev_testing_app/Model/EngineerModel/EngineerVerifyModel.dart';
...
class EngineerVerify extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      home: EngineerVerifyScreen(),
    );
  }
}

class EngineerVerifyScreen extends StatefulWidget {
  

  @override
  _EngineerVerifyScreenState createState() => _EngineerVerifyScreenState();
}

class _EngineerVerifyScreenState extends State<EngineerVerifyScreen> {
  ...
  Future<EngineerVerifyModel> veryfyEngineerProcess() async {
    var engineerVerify_url= engineerVerifyApi;
    final http.Response response = await http.post(
        Uri.parse(engineerVerify_url),
    ...
    if (response.statusCode == 200) {
      ...
      print('verify Engineer body');
      print(response.body);
       return EngineerVerifyModel.fromJson(json.decode(response.body));
    } else {
      ...
      onPressed: () async{
        ...
        if(_formkey.currentState!.validate()){
          try{
            ...
            var response=await veryfyEngineerProcess();
            bool res=response.status;
            message =response.message.toString();
            var engineerid=response.engineerid.toString();
            if(res==true){

              //updateMsg();

              EngineerVerify.engineerid=engineerid.toString();
              ...
              Navigator.of(context,rootNavigator: true).push(MaterialPageRoute(builder: (context)=>EngineerOtpVerify()));
            }
            if(res==false){
            ...
            child: Center(
               child: FittedBox(
                child: Padding(
                  ...
                  child: Text("Verify Engineer ",style: TextStyle(fontFamily: 'AkayaKanadaka',fontSize: 30,fontWeight: FontWeight.w700,color: themWhiteColor),),
                ),
              ),
            ),
          ),
